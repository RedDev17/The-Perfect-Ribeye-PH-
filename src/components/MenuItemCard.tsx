...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ 
  ...
  return (
    <>
      <div className={`bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-lg hover:shadow-2xl hover:shadow-red-900/20 transition-all duration-300 overflow-hidden group animate-scale-in border border-gray-700/50 hover:border-red-800/50 ${!item.available ? 'opacity-60' : ''}`}>
        ...
        <div className="relative h-48 bg-gradient-to-br from-gray-900 to-gray-800">
          ...
          <div className={`absolute inset-0 flex items-center justify-center ${item.image ? 'hidden' : ''}`}>
            <div className="text-6xl opacity-30 text-gray-600">🥩</div>
          </div>
          
          {/* Badges */}
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {item.isOnDiscount && item.discountPrice && (
              <div className="bg-gradient-to-r from-red-600 to-red-700 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg animate-pulse">
                🔥 SALE
              </div>
            )}
            {item.popular && (
          ...
          {!item.available && (
            <div className="absolute top-3 right-3 bg-gray-800/90 text-red-400 text-xs font-bold px-3 py-1.5 rounded-full shadow-lg border border-red-800/50">
              UNAVAILABLE
            </div>
          )}
          ...
          {item.isOnDiscount && item.discountPrice && (
            <div className="absolute bottom-3 right-3 bg-gray-900/90 backdrop-blur-sm text-red-400 text-xs font-bold px-2 py-1 rounded-full shadow-lg border border-red-800/30">
        ...
        <div className="p-5">
          <div className="flex items-start justify-between mb-3">
            <h4 className="text-lg font-semibold text-white leading-tight flex-1 pr-2">{item.name}</h4>
            {item.variations && item.variations.length > 0 && (
              <div className="text-xs text-gray-400 bg-gray-700/50 px-2 py-1 rounded-full whitespace-nowrap border border-gray-600/30">
                {item.variations.length} sizes
              </div>
            )}
          </div>
          
          <p className={`text-sm mb-4 leading-relaxed ${!item.available ? 'text-gray-500' : 'text-gray-400'}`}>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex-shrink-0">
              {!item.available ? (
                <button
                  disabled
                  className="bg-gray-700 text-gray-500 px-4 py-2.5 rounded-xl cursor-not-allowed font-medium text-sm border border-gray-600"
                >
                  Unavailable
                </button>
              ) : quantity === 0 ? (
                <button
                  ...
                  className="bg-gradient-to-r from-red-600 to-red-700 text-white px-6 py-2.5 rounded-xl hover:from-red-700 hover:to-red-800 transition-all duration-200 transform hover:scale-105 font-medium text-sm shadow-lg hover:shadow-red-900/30"
                >
                ...
                <div className="flex items-center space-x-2 bg-gray-700/50 rounded-xl p-1 border border-gray-600/50">
                  ...
                  <span className="font-bold text-white min-w-[28px] text-center text-sm">{quantity}</span>
                  <button
          ...
          {item.addOns && item.addOns.length > 0 && (
            <div className="flex items-center space-x-1 text-xs text-gray-400 bg-gray-700/30 px-2 py-1 rounded-lg border border-gray-700/50">
              <span>+</span>
